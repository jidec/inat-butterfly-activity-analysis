...
cells
cell_id <- 12 #washington DC 178 #12 south texas 
```

Optionally: filter to one cell

```{r}

obs_data <- filter(obs_data, cell == cell_id)
...
one_cell_data <- one_cell_data %>%
  mutate(season = factor(season, levels = names(season_labels), labels = season_labels))
```
...
table(obs_data$species) 

### Subsample -- test species. ----
testspecies <- obs_data %>%
  dplyr::filter(species == "Hylephila phyleus")
...
# and you've already filtered times to 7–20 in that dataset or want the function to do so.
