...
m <- getPlotBRMS(response="duration",formula="ns(temp_sc,2) + wingspan_sc + temp_sc:wingspan_sc + daylength_sc + (1|cell)",tree=tree,data=sscs)

plot_model(m, type="int",terms = c("temp_sc", "wingspan_sc"),show.data=TRUE)
...
# model <- brm(response ~ ns(temp_sc, 2) + (1 | group), data = your_data, family = "gaussian")

your_data <- sscs
model <- m
# Generate new data
new_data <- data.frame(
  temp_sc = seq(min(your_data$temp_sc), max(your_data$temp_sc), length.out = 100),
  wingspan_sc = mean(your_data$wingspan_sc, na.rm = TRUE),  # Set to mean or another value
  daylength_sc = mean(your_data$daylength_sc, na.rm = TRUE) # Set to mean or another value
)

# Predict without random effects
preds <- posterior_epred(model, newdata = new_data, re_formula = NA)

# Summarize predictions
pred_summary <- data.frame(
  temp_sc = new_data$temp_sc,
  fit = apply(preds, 2, mean),
  lower = apply(preds, 2, function(x) quantile(x, 0.025)),
  upper = apply(preds, 2, function(x) quantile(x, 0.975))
)

# Plot
library(ggplot2)
ggplot(pred_summary, aes(x = temp_sc, y = fit)) +
  geom_line() +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.2) +
  labs(x = "Scaled Temperature (temp_sc)", y = "Predicted Response", title = "Effect of Temperature (Spline)") +
  theme_minimal()
